create table PROMOTION (ID bigint not null, PROMOTION_TYPE varchar(255), CODE varchar(255), USERACCOUNT bigint, USED bit, PRODUCT bigint, VALID_FROM timestamp, VALID_TO timestamp, TIME_STAMP timestamp, primary key (ID));
...
alter table MEMBERSHIP add column PROMOTION bigint;
alter table MEMBERSHIP add constraint FKCD0773D6AE15FA3B foreign key (PROMOTION) references PROMOTION;

/* 
 * Update MEMBERSHIP.PROMOTION:
 * setze Promotion-ID, falls Membership in Zusammenhang mit einer Promotion abgeschlossen wurde.
 */
/*update MEMBERSHIP set PROMOTION=(select ID from PROMOTION as p, MEMBERSHIP as m where m.USERACCOUNT_ID=p.USERACCOUNT and m.PROMOTION > 0);*/
